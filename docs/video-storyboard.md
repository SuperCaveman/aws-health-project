...
3. **Successful intake (0:50–1:35):** Request a short-lived upload URL as an authenticated user, then submit a licensed de-identified DICOM study; show validation, classification, source storage, and HealthImaging import.
...
- Record now: the quarantined fake `.dcm` and its audit record; the successful Step Functions provenance execution; and the two unauthenticated API calls returning HTTP 401.
- Record after adding a permitted Cognito demo user: a successful short-lived upload URL and authenticated derived-asset download.
- Record after adding licensed de-identified DICOM: HealthImaging import completion and the automatic EventBridge-to-Step-Functions run.
